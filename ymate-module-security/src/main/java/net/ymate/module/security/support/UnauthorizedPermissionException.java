...
public class UnauthorizedPermissionException extends RuntimeException {
    ...
    private static String doBuildMessage(IUserInfo user, String[] permissions) {
        return String.format("User[id=%s] permissions are not within the allowed range: [%s]", user.getId(), StringUtils.join(permissions, ", "));
    }
    ...
    public UnauthorizedPermissionException(IUserInfo user, String[] permissions) {
        super(doBuildMessage(user, permissions));
        this.user = user;
